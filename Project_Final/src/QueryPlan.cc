...
void QueryPlan::start()
{
	...
	{
		cout<<"Query Plan :\t Executing ..."<<endl;
		ExecuteTree(root);
	}
}
...
int QueryPlan::clear_pipe (Pipe &in_pipe, Schema *schema, bool print)
{
	...
	while (in_pipe.Remove (&rec)) {
		cnt++;
		cout<<"Remove from In-Pipe \n\t (Record No):"<<cnt<<endl<<endl;
		if (print) {
			rec.Print (schema);
		}
...
AndList* QueryPlan:: CreateAndList( vector<char*> &input)
{
	...
	cout<<"Query Plan :\t CreateAndList \t Input Size = "<<input.size()<<endl;
	for(int i=0;i<input.size();i++)
	{
		cout<<"Query Plan :\t CreateAndList \t Input = "<<input[i]<<endl;
...
void QueryPlan::PrintTree(BaseNode* node)
{
	...
	case 1: cout<<"Query Plan :\tSelectFile\n\n";
	cout<<"Query Plan :\tRelation Name:"<<node->nodeTables<<endl;
	cout<<"Query Plan :\n\t\tOutput Pipe ID:"<<node->nodepipeID<<endl;
	PrintSchema(node->nodeSchema);
	cout<<"Query Plan :\tSelect CNF:\n";
	node->nodeCnf->Print();

	break;

	case 2: cout<<"Query Plan :\tSelectPipe\n\n";
	break;

	case 3: cout<<"Query Plan :\tProject\n\n";
	cout<<"Query Plan :\n\t\tInput Pipe ID:"<<node->left->nodepipeID<<endl;
	cout<<"Query Plan :\n\t\tOutput Pipe ID:"<<node->nodepipeID<<endl;
	PrintSchema(node->nodeSchema);
	break;

	case 4: cout<<"Query Plan :\tJoin Operation\n\n";
	cout<<"Query Plan :\n\t\tInput Pipe1 ID:"<<node->left->nodepipeID<<endl;
	cout<<"Query Plan :\n\t\tInput Pipe2 ID:"<<node->right->nodepipeID<<endl;
	cout<<"Query Plan :\n\t\tOutput Pipe ID:"<<node->nodepipeID<<endl;
	PrintSchema(node->nodeSchema);
	cout<<"Query Plan :\t Join CNF predicate:\n\t\t";
	node->nodeCnf->Print();
	break;

	case 5: cout<<"Query Plan :\tDuplicate Removal\n\n";
	cout<<"Query Plan :\n\t\tInput Pipe ID:"<<node->left->nodepipeID<<endl;
	cout<<"Query Plan :\n\t\tOutput Pipe ID:"<<node->nodepipeID<<endl;
	break;

	case 6: cout<<"Query Plan :\t Sum Operation\n\n";
	PrintSchema(node->nodeSchema);
	break;
	...
	cout<<"Query Plan :\n\t\tOutput Pipe ID:"<<node->nodepipeID<<endl;
	PrintSchema(node->nodeSchema);
	cout<<"Query Plan :\tSorting Order\n";
	cout << "\t";
	node->groupAtts->Print();
	break;

	default: cout<<"Query Plan :\tDefault Operation Selected\n";
	break;
	}
...
{
	cout <<"Query Plan :\tSelecting File..."<<endl;
...
void QueryPlan:: execP(BaseNode *node)
{
	...
	if(distinctAtts != 1)
	{
		if(outType == 1)
		{
			...
			P.WaitUntilDone();
			cout << "\n\nThe number of records returned by query is: " << cnt << "\n\n" << endl;
		}
		if(outType == 2)
		{
